...
class relu(nn.ReLU):
    ...
    def forward(x, num_bits, num_grad_bits):
        return 
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, num_bits=8):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, precision_mode, num_classes=10):
        self.inplanes = 16
        ...
        self._make_group(block, 16, layers[0], precision_mode[0], group_id=1
                         )
        self._make_group(block, 32, layers[1], precision_mode[1], group_id=2
                         )  
        self._make_group(block, 64, layers[2], precision_mode[2], group_id=3
                         )
    ...
    def _make_group(self, block, planes, layers, precision, group_id=1 
                    ):
        ...
        for i in range(layers):
            ...
            layer = self._make_layer_v2(block, planes, k, stride=stride,
                                       )
            setattr(self, 'group{}_layer{}_bit{}'.format(group_id, i,k), layer)
            print(self.__dict__)

    def _make_layer_v2(self, block, planes, num_bits,stride=1, 
                       ):
        ...
        layer = block(self.inplanes, planes, stride, downsample, num_bits)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, num_bits))
        self.inplanes = planes * block.expansion
    ...
    def forward(self, x, precision_mode, num_bits, num_grad_bits):
        ...
        for g in range(3):
            ...
            for i in range(self.num_layers[g]):
                x = getattr(self, 'group{}_layer{}_bit{}'.format(g+1, i, k))(x, num_bits, num_grad_bits)
...
def cifar100_resnet_38(pretrained=False, **kwargs):
    # n = 6
    model = ResNet(BasicBlock, [6, 6, 6], [2, 4, 8], num_classes=100)
    return model
