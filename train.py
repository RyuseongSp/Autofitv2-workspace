...
def parse_args():
    ...
    parser.add_argument('--is_precision_scheduled', action='store_true',
                        help='cyclic precision schedule')
    parser.add_argument('--precision_schedule', default=None, type=int, nargs='*',
                        help='cyclic schedule for weight/act precision')
    parser.add_argument('--precision_grad_bits_schedule', default=None, type=int, nargs='*',
    ...
    parser.add_argument('--inference_bits', default=None, type=int, nargs='*', 
                        help='precision schedule for inference')
...
def main():
    ...
    if args.cmd == 'train':
        ...
        args.inference_bits = args.precision_schedule
        run_training(args)
        ...
        logging.info('start inference {} with precision {}'.format(
            args.arch, args.inference_bits))
        run_inference(args)
...
def run_training(args):
    ...
    while i < args.iters:
        for input, target in train_loader:
            ...
            cyclic_period = 1
            precision_schedule(args, i, cyclic_period)
            ...
            # compute output
            output = model(input_var, args.inference_bits, args.num_bits, args.num_grad_bits)
...
def precision_schedule(args, _iter, cyclic_period):
    if args.is_precision_scheduled:
        assert len(args.precision_schedule) == 3
        assert len(args.precision_grad_bits_schedule) == 2

        num_grad_bit_min = args.precision_grad_bits_schedule[0]
        num_grad_bit_max = args.precision_grad_bits_schedule[1]

        args.num_bits = args.precision_schedule[int(_iter//(args.iters/3))]
        args.num_grad_bits = num_grad_bit_max
        if _iter % args.eval_every == 0:
            logging.info('Iter [{}] num_bits = {} num_grad_bits = {} precision'.format(_iter, args.num_bits,
                                                                                                  args.num_grad_bits))
